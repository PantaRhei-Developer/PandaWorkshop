...
1. **初期設計書の作成**
   ...
   - (河野さんレビュー2025/09/29)
     - 最初に作るのは要件定義書
     - 初期設計書は要件定義書に当たる。(自社開発だと要件定義書は要らないかも←)
     - ケースバイケースでワークフローが決まっているわけではない
     - この順番でやっていけば良いよというのはない
     - プロジェクトの内容によって順番が変わってくる
     - めちゃくちゃ小さな案件はこれはやりすぎ
     - 数千万数億の案件だとこれは足りない
     - 中間ボリュームの進め方の進め方のよう(1000万~2000万くらい)
     - webなのかアプリなのか、技術要件は何なのか
     - 全てのプロジェクトに対して当てはめるのは認識が違う
     - ちゃんとしたコアの部分が把握できていない()
     - 最適なワークフローを考えないといけない(実案件があるとナレトレしやすい)
     - 要件定義書の組み立てと粒度など
       - エンジニアによって品質がブレる
     - これでいいんですかと質問がくる、この資料だと
     - 人のスキルが介在してくる
     - レビューをできる人が必要になってくる。
